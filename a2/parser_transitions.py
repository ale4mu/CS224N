...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if(transition == "S"):
            ...
            # 栈顶第二个元素指向第一个元素，第一个元素弹出
            if(len(self.stack) >= 2):
                depend = self.stack.pop()
                head = self.stack[-1]
                self.dependencies.append((head, depend))
        else:
            # 栈顶元素指向第二个元素，第二个元素弹出
            if(len(self.stack) >= 2):
                head = self.stack.pop()
...
def minibatch_parse(sentences, model, batch_size):
    ...
    """
    dependencies = []
    partial_parses = [PartialParse(sent) for sent in sentences] # 每句分别初始化一个模型
    unfinished_parses = list(partial_parses) # 避免浅拷贝
    while unfinished_parses:
        batch = unfinished_parses[:batch_size]
        transitions = model.predict(batch) # 预测要执行的transition
        for parse,trans in zip(batch,transitions):
            parse.parse_step(trans) # 执行transition
        unfinished_parses =[p for p in unfinished_parses if not(len(p.buffer) == 0 and len(p.stack) == 1)]
    dependencies = [p.dependencies for p in partial_parses] # 每个模型的依赖存储在self.dependencies中
