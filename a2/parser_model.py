...
class ParserModel(nn.Module):
    ...
    def embedding_lookup(self, w):
        ...
        w_flat = w.view(-1) # 展平，index_select要求索引是一维的
        x = torch.index_select(self.embeddings,0,w_flat) # 从self.embedding中按行(dim=0)选取指定索引的行（词向量)
        # (batch_size * n_features, embed_size) -> (batch_size, n_features * self.embed_size)
    ...
    def forward(self, w):
        ...
        logits = None
        x1 = self.embedding_lookup(w) # embedding, (batch_size, n_features * embed_size)
        x2 = torch.matmul(x1, self.embed_to_hidden_weight)+ self.embed_to_hidden_bias
        x3 = F.relu(x2)
        x4 = self.dropout(x3)
        logits = torch.matmul(x4,self.hidden_to_logits_weight) + self.hidden_to_logits_bias
        ### END YOUR CODE
        return logits
