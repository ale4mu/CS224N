...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        # 滑动窗口卷积
        # 捕捉局部n-gram特征,padding="same"自动补齐使输入和输出相同
        self.post_embed_cnn = nn.Conv1d (embed_size,embed_size,2,padding="same") 
        self.encoder = nn.LSTM(embed_size,hidden_size,bidirectional=True,dropout=dropout_rate) 
        # 单步 LSTM 单元，用于逐时间步生成目标序列；输入为嵌入层和上一步attention的输出拼接
        self.decoder = nn.LSTMCell(embed_size+hidden_size,hidden_size)
        # 将双向编码器的最终隐藏状态投影到解码器初始隐藏状态
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        source_lengths = [len(s) for s in source] # 实际长度
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int], grader_params=None) -> Tuple[
        ...
        # print("source_padded shape:", source_padded.shape)  # 应为 (src_len, b)
        # print("X shape after embedding:", X.shape)         # 应为 (src_len, b, e)

        X_permute = torch.permute(X,(1,2,0))  #(b,e,src_len)
        X_conv = self.post_embed_cnn(X_permute)
        X = torch.permute(X_conv,(2,0,1))  #(src_len, b, e)

        # 告诉LSTM哪些是真实词，哪些是padding
        X = nn.utils.rnn.pack_padded_sequence(X, source_lengths)  
        enc_hiddens,(last_hidden,last_cell) = self.encoder(X)
        enc_hiddens, src_len = nn.utils.rnn.pad_packed_sequence(enc_hiddens)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for i in range(Y.size(0)):
            Y_t = Y[i]  #techer forcing
            Ybar_t = torch.cat((Y_t, o_prev), dim=-1)  
            dec_state, o_t, e_t = self.step(Ybar_t, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks)
            combined_outputs.append(o_t) # (b,h) 组合输出，加权完经过全连接层和tanh
            o_prev = o_t
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # Set e_t to -inf where enc_masks has 1（看不到未来），这样在softmax中这些项为0
        if enc_masks is not None:
            e_t.data.masked_fill_(enc_masks.bool(), -float('inf'))

        alpha_t = torch.nn.functional.softmax(e_t,dim=-1) # (b, src_len) # 权重
