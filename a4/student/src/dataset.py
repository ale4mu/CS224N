...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        ### YOUR CODE HERE ###
        doc = self.data[idx]
        #truncate_len = random.randint(4,min(len(doc),self.block_size*7//8))
        upper_bound = min(len(doc), self.block_size * 7 // 8)
        lower_bound = 4
        if upper_bound < lower_bound:
            truncate_len = upper_bound
        else:
            truncate_len = random.randint(lower_bound, upper_bound)
        truncate_doc = doc[:truncate_len]

        mean_len = truncate_len // 4
        std_len = mean_len // 2
        masked_len = int(random.gauss(mean_len,std_len)) # 可能出错
        while masked_len < 1 or masked_len > truncate_len - 2:
            masked_len = int(random.gauss(mean_len,std_len))
        start_idx = random.randint(0,truncate_len-masked_len-1)

        prefix = truncate_doc[:start_idx]
        masked_content = truncate_doc[start_idx:start_idx+masked_len]
        suffix = truncate_doc[start_idx+masked_len:]

        masked_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content
        n_pad = self.block_size - len(masked_string) - 1
        masked_string += self.PAD_CHAR * n_pad
