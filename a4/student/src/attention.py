...
# RoPE(Rotary Position Embedding，旋转位置编码)编码序列中的位置信息
# 计算 Q 和 K 的点积时保留相对位置关系
def precompute_rotary_emb(dim, max_positions):
    ...
    #   创建频率索引 i 的张量。直接计算公式中的 2*(i-1) 部分。
    #    对于从0开始的索引 k = i-1，我们需要 2*k，即 [0, 2, 4, ..., dim-2]
    #    形状: (dim / 2)
    inv_freq_indices = torch.arange(0, dim, 2, dtype=torch.float32)

    # 计算所有的 theta_i 值: 10000^(-2k/dim)
    #    形状: (dim / 2)
    thetas = 10000.0 ** (-inv_freq_indices / dim)

    # 使用广播机制 (torch.outer) 计算所有 t * theta_i 的组合
    #    torch.outer(positions, thetas) 会得到一个矩阵，
    #    其中 freqs[t, i] = positions[t] * thetas[i]
    #    形状: (max_positions, dim / 2)
    freqs = torch.outer(positions, thetas)

    # 5. 一次性计算所有的 cos 和 sin 值
    #    形状均为: (max_positions, dim / 2)
    cos_vals = torch.cos(freqs)
    sin_vals = torch.sin(freqs)

    # 6. 将 cos 和 sin 值堆叠起来，形成最终的形状
    #    在最后一个维度上堆叠，得到形状 (max_positions, dim / 2, 2)
    rope_cache = torch.stack([cos_vals, sin_vals], dim=-1)
    return rope_cache

# 将预计算好的旋转位置编码应用到输入张量 x 上
def apply_rotary_emb(x, rope_cache):
    ...
    ### YOUR CODE HERE ###
    B,H,T,D = x.shape
    x = x.view(B,H,T,D//2,2) #(B,H,T,D//2,2)
    x = torch.view_as_complex(x) # (B,H,T,D//2)
    rope_cache = rope_cache[:T] # 截断到实际长度
    rope_cache = torch.view_as_complex(rope_cache)
    # cos = rope_cache[:,:,0]
    # sin = rope_cache[:,:,1]
    # cos = cos.view(1,1,T,D//2) 
    # sin = sin.view(1,1,T,D//2)
    #rotate = torch.view_as_complex(torch.stack([cos,sin],dim=-1))
    rotated_x = x * rope_cache # 复数乘法
    rotated_x = torch.view_as_real(rotated_x) 
    rotated_x = rotated_x.reshape(B, H, T, D)
    ### END YOUR CODE ###
    return rotated_x
...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        # register_buffer：不参与梯度更新，但随模型加载
        self.register_buffer("mask", torch.tril(torch.ones(config.block_size, config.block_size))
                                     .view(1, 1, config.block_size, config.block_size)) #torch.tril 生成下三角矩阵
